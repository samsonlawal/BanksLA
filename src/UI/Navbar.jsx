...
export default function Navbar() {
  ...
  const toggleMenu = () => {
    ...
    hamburgerX.classList.toggle("flex");
    hamburgerX.classList.toggle("hidden");

    hamburgeBtn.classList.toggle("hidden");
    hamburgeBtn.classList.toggle("flex");
  };
  ...
  return (
    <nav className="h-20 md:h-24 font-mont text-base">
      <div className="w-full h-full flex flex-row justify-between lg:justify-evenly items-center font-medium pr-4 md:pr-6 lg:pr-0">
        {/* Logo */}
      ...
      <section
        id="mobile-menu"
        className="bg-white absolute top-20 z-20 px-6 w-full flex-col justify-center items-center origin-top animate-open-menu font-medium hidden h-[calc(100%-80px)] py-4 text-lg font-mont lg:hidden"
      >
        {/* links ad buttons */}
        <div className="w-full h-fit flex flex-col items-center  space-y-8">
          ...
          <ul className="h-full flex flex-col lg:flex-row lg:space-x-12 justify-center items-center px-10 space-y-8">
            ...
            <button className="px-8 y-4 text-primary bg-secondary py-2 hover:text-white hover:bg-primary">
              Locations
            </button>
          </ul>
        </div>
